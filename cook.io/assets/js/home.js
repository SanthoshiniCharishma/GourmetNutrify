...
$searchBtn.addEventListener("click",function(){
    if($searchField.value) window.location=`/cook.io/recipes.html?q=${$searchField.value}`;
});
...
const addTabContent=($currentTabBtn,$currentTabPanel)=>{
   ...
   fetchData([['mealType',$currentTabBtn.textContent.trim().toLowerCase
    ()], ...cardQueries],function(data){
        ...
        $currentTabPanel.innerHTML+=`
           <a href="./recipes.html?mealType=${$currentTabBtn.textContent.trim().toLowerCase()}" class="btn btn-secondary label-large
...
//          id="slider-label-1">Latest ${cuisineType[index]} Recipies
//          </h2>
...
//                            <a href="./recipes.html?cuisineType=${cuisineType[index].toLo}" class="load-more-card has-state">
...
for (const [index, $sliderSection] of $sliderSections.entries()) {
    ...
    <div class="container">
        <h2 class="section-title headline-small"
         id="slider-label-1">Latest ${cuisineType[index]} Recipes
         </h2>

      <div class="slider">
        <ul class="slider-wrapper" data-slider-wrapper-${index}>
        ${`<li class="slider-item">${$skeletonCard}</li>`.repeat(10)}
        </ul>
      </div>
    </div>
    ...
    const $sliderWrapper = $sliderSection.querySelector(`[data-slider-wrapper-${index}]`);

    // Fetch the data and add items to the slider without wiping out existing content
    fetchData([...cardQueries, ["cuisineType", cuisineType[index]]], function (data) {
        // Cache current scroll position
        const currentScrollPosition = $sliderWrapper.scrollLeft;
        
        // Don't clear the content, just add to it
        // Remove the skeleton cards first
        $sliderWrapper.innerHTML = "";

        // Loop through the fetched data and append new slider items
        data.hits.map(item => {
            const {
        ...
        $sliderWrapper.scrollLeft = currentScrollPosition;
    });
}
